...
import blockquote from '../../../next.svg';
import user1 from '../../../../../assets/images/testimonials/user-1.png';

const TestimonialSlider = (props) => {
  const sliderRef = useRef(null);
  const [currentSlide, setCurrentSlide] = useState(0);
  const length = 4; // length of data
  const settings = {
    dots: false,
    infinite: false,
    speed: 500,
          ...
          ? props.data.map((t, index) => (
              <div key={index}>
                <div className="flex flex-col items-center w-full gap-10 md:flex-row">
                  ...
                  <div className="relative h-full pb-8 mt-4 md:mt-0">
                    <svg
                      width="120"
                      height="100"
                      viewBox="0 0 120 102"
                      ...
                      className="absolute w-[120px] h-[100px] top-2"
                    >
                      <g opacity="0.06">
                        <path
                    ...
                    <p className="ml-20 opacity-80">{t.description}</p>
                    {/* <h4>{t.title}</h4> */}
                  </div>
                </div>
              </div>
      ...
      <div className="flex items-center justify-center w-full gap-2 md:justify-end">
        <ArrowLeft
          ...
          className={cn('cursor-pointer', {
            'text-opacity-50 text-secondary-500 cursor-not-allowed':
              currentSlide === 0,
            'hover:text-primary-500': currentSlide > 0,
          })}
        />
        <ArrowRight
          onClick={() => {
          ...
          className={cn('cursor-pointer', {
            ...
            'hover:text-primary-500': currentSlide < length - 1,
          })}
        />
      </div>
    </div>
